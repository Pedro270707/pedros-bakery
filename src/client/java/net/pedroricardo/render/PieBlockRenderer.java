...
public class PieBlockRenderer implements BlockEntityRenderer<PieBlockEntity> {
	...
	public void render(PieBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		...
		for (int i = 0; i < slices; i++) {
			bottom[i].render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityCutout(texture)), light, getBakeTimeOverlay(entity.getBottomBakeTime(), overlay), getBakeTimeColor(entity.getBottomBakeTime(), 0xFFFFFFFF));
		}
		if (layers >= 2) {
		...
		if (layers == 3) {
			for (int i = 0; i < slices; i++) {
				top[i].render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityCutoutNoCullZOffset(texture)), light, getBakeTimeOverlay(entity.getTopBakeTime(), overlay), getBakeTimeColor(entity.getTopBakeTime(), 0xFFFFFFFF));
			}
		}
	}
