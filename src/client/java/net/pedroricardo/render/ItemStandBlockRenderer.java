...
public class ItemStandBlockRenderer implements BlockEntityRenderer<ItemStandBlockEntity> {
    ...
    public ItemStandBlockRenderer(BlockEntityRendererFactory.Context ctx) {
    ...
    public void render(ItemStandBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (!state.getOrEmpty(ItemStandBlock.HAS_ITEM).orElse(false)) return;
        matrices.push();
        if (entity.getStack().getItem() instanceof BlockItem) {
            ...
            matrices.translate(0.0f, -0.0625f, 0.0f);
            this.itemRenderer.renderItem(entity.getStack(), ModelTransformationMode.GROUND, light, overlay, matrices, vertexConsumers, entity.getWorld(), (int) entity.getPos().asLong());
        } else {
            matrices.translate(0.0f, -0.5f, 0.0f);
