...
public class PBRenderHelper {
    public static void createFace(Direction direction, PoseStack matrices, VertexConsumer vertexConsumer, float x, float y, float z, float width, float height, float u, float v, int light, int overlay, int color) {
        createFace(direction, matrices, vertexConsumer, x, y, z, width, height, u, v, u + width, v + height, light, overlay, color);
    }

    public static void createFace(Direction direction, PoseStack matrices, VertexConsumer vertexConsumer, float x, float y, float z, float width, float height, float u, float v, float u2, float v2, int light, int overlay, int color) {
        createFace(direction, matrices, vertexConsumer, x, y, z, width, height, u, v, u2, v2, 128.0f, 128.0f, light, overlay, color);
    }

    public static void createFace(Direction direction, PoseStack matrices, VertexConsumer vertexConsumer, float x, float y, float z, float width, float height, float u, float v, float u2, float v2, float textureWidth, float textureHeight, int light, int overlay, int color) {
        matrices.pushPose();
        PoseStack.Pose entry = matrices.last();
        Vector3f rotationEuler = direction.getRotation().getEulerAnglesXYZ(new Vector3f());
...
//        Vector3f vector3f2 = entry.normal().transform(new Vector3f(quad.direction));
...
//            Vector3f vector3f3 = entry.pose().transformPosition(i, j, k, vector3f);
...
//        }
        ...
        entry.pose().rotate(rotation.conjugate());
        matrices.popPose();
    }
}
