...
public class PBBlocks {
    ...
    public static final Block BEATER = register("beater", new BeaterBlock(AbstractBlock.Settings.create().sounds(BlockSoundGroup.METAL).solid().hardness(1.5f).resistance(1.5f).mapColor(MapColor.OFF_WHITE).requiresTool()));
    public static final Block BAKING_TRAY = register("baking_tray", new BakingTrayBlock(AbstractBlock.Settings.create().sounds(BlockSoundGroup.METAL).solid().hardness(1.0f).resistance(1.0f).mapColor(MapColor.OFF_WHITE).requiresTool().dynamicBounds().nonOpaque().solidBlock(Blocks::never).blockVision(Blocks::never)), (name, block) -> PBItems.register(name, new BakingTrayItem(block, new Item.Settings().maxCount(1).component(PBComponentTypes.BATTER, Collections.singletonList(CakeLayer.getEmpty())).component(PBComponentTypes.SIZE, PedrosBakery.CONFIG.bakingTrayDefaultSize()).component(PBComponentTypes.HEIGHT, PedrosBakery.CONFIG.bakingTrayDefaultHeight()))));
